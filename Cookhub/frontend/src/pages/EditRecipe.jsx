...
function EditRecipe() {
	...
	const handleSave = async () => {
		...
		if (!recipeName.trim()) { alert("Please enter a recipe name"); return; }
		...
		try {
			...
			if (!recipeRes.ok) throw new Error("Failed to update recipe");
	...
	useEffect(() => {
		...
		.then(data => {
			...
			setNumberOfPersons(data.number_of_persons || "");
			setIngredients(data.ingredients.map(ing => ({
				...ing,
				pricePerUnit: ing.quantity ? ing.price / ing.quantity : ""
			})));
			setLoading(false);
		})
	}, []);
