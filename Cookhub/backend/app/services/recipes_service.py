...
class RecipesFacade:
    ...
    def recalculate_recipe_price(self, recipe_id):
        recipe = self.recipes_repo.get(recipe_id)
        if not recipe:
            raise ValueError("Recipe not found")
        total = sum(ing.price for ing in recipe.ingredients)
        recipe.total_price = total
    ...
    def get_price_recipe(self, recipe_id):
        ...
        total_price = 0
        total_price = sum(ing.price for ing in ingredients_recipe)
        return total_price
